    ...
    #   compute derivative with central diffs
        ...
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                # change only one scalar dimension in W at a time
                a = np.zeros(v.shape)
                a[i, j] = 1

                params1 = copy.deepcopy(params)
                params1[k] = v + eps * a
                post_act1 = forward(xb, params1, 'layer1')
                probs1 = forward(post_act1, params1, 'output', softmax)
                loss1, acc1 = compute_loss_and_acc(yb, probs1)

                params2 = copy.deepcopy(params)
                params2[k] = v - eps * a
                post_act2 = forward(xb, params2, 'layer1')
                probs2 = forward(post_act2, params2, 'output', softmax)
                loss2, acc2 = compute_loss_and_acc(yb, probs2)

                params['grad_' + k][i, j] = (loss1 - loss2) / (2 * eps)

    elif 'b' in k:
        for i in range(v.shape[0]):
            # change only one scalar dimension in b at a time
            a = np.zeros(v.shape)
            a[i] = 1
